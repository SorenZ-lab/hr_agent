...
async def upload_resume(
    ...
    async with AsyncSessionLocal() as session:
        await session.execute(
            text("""
                INSERT INTO resume_reviews (id, tenant_id, candidate_id, pdf_minio_path, status)
                VALUES (:id, :tenant_id, :candidate_id, :pdf_minio_path, 'processing')
            """),
            {"id": review_id, "tenant_id": tenant_id, "candidate_id": candidate_id,
             "pdf_minio_path": f"resumes/{candidate_id}/{review_id}.pdf"},
        )
        await session.commit()

    # 3. 准备初始 State，后台启动图执行
    initial_state = {
        "messages": [], "candidate_id": candidate_id, "tenant_id": tenant_id,
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            text("""
                ...
                FROM resume_reviews
                WHERE id = :review_id AND candidate_id = :candidate_id
            """),
            {"review_id": review_id, "candidate_id": current_user["user_id"]},
        )
...
async def delete_review(review_id: str, current_user: dict = Depends(get_current_user)):
    """删除审查记录（WHERE 带 candidate_id，只能删自己的）。"""
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            text("""
