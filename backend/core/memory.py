...
def get_memory_saver(agent_type: str = "default") -> MemorySaver:
    ...
    Args:
        agent_type: Agent 标识符，如 "qa" / "resume"
...
def build_thread_id(candidate_id: str, session_id: str) -> str:
    """
    构建 LangGraph Checkpointer 使用的 thread_id。

    格式：candidate_{candidate_id}_session_{session_id}
    同一候选人的不同会话有独立的历史，互不干扰。

    Example:
        build_thread_id("abc123", "xyz789")
        → "candidate_abc123_session_xyz789"
    """
    return f"candidate_{candidate_id}_session_{session_id}"


def build_config(candidate_id: str, session_id: str) -> dict:
    """
    构建 LangGraph 调用所需的 config 字典。

    用法：
        config = build_config(candidate_id, session_id)
        result = await graph.ainvoke(state, config=config)

    Returns:
        {"configurable": {"thread_id": "candidate_xxx_session_yyy"}}
    """
    return {
        "configurable": {
...
async def compress_to_summary(
    ...
    SUMMARY_PROMPT = """请将以下候选人对话压缩为结构化候选人画像摘要。

【压缩规则】
必须保留：候选人明确不理解的知识点 / 反复出现的薄弱点 / 项目背景新增信息
...
请直接输出摘要文本，不要加任何前缀。"""
    ...
    conversation_text = "\n".join([
        f"{'候选人' if isinstance(m, HumanMessage) else 'AI'}：{m.text if hasattr(m, 'text') and not callable(m.text) else str(m.content)}"
        for m in messages
...
if __name__ == '__main__':
    # print(build_config(candidate_id="abc123", session_id="xyz789"))
    # print(get_memory_saver(agent_type="qa"))
    # ── 测试 2：滑动窗口裁剪 ─────────────────────────────────────
    messages = [SystemMessage(content="你是招聘助手")]
