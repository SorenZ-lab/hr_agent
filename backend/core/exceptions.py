...
# HRAgent 统一异常体系：所有自定义异常都继承同一个基类，
# 便于「一次捕获全部」以及按「可重试 / 不可重试」分类处理。


class HRAgentBaseError(Exception):
    """所有 HRAgent 自定义异常的基类（继承 Python 内置的 Exception）。
    比普通异常多带两样上下文：是哪个 Agent 出的错、以及任意细节字典。"""
    def __init__(self, message: str, agent_type: str = "", details: dict = None):
        # message：错误描述文字；agent_type：哪个 Agent（qa/resume）；details：额外细节
...
class LLMAPIError(HRAgentBaseError):
    """大模型 API 调用失败（超时 / 限流 / 网络错误）。属于【可重试】异常。"""
    pass                                   # 无需额外逻辑，直接继承基类的能力即可


class AgentExecutionError(HRAgentBaseError):
    """Agent 业务逻辑执行失败。"""
    pass


class PipelineError(HRAgentBaseError):
    """多 Agent Pipeline（流水线编排）失败。"""
    pass


class IntentRouteError(HRAgentBaseError):
    """意图识别路由失败（没判断出该交给哪个 Agent）。"""
    pass


class MilvusConnectionError(HRAgentBaseError):
    """Milvus 向量库连接失败。属于【可重试】异常。"""
    pass


class FileParseError(HRAgentBaseError):
    """文件解析失败（Word / PDF）。"""
    pass


class InvalidInputError(HRAgentBaseError):
    """用户输入不合法。属于【不可重试】异常（重试也不会变合法）。"""
    pass


class AuthenticationError(HRAgentBaseError):
    """认证失败。属于【不可重试】异常。"""
    pass
if __name__ == '__main__':
    ...
    print("LLMAPIError 是 HRAgentBaseError 子类:", issubclass(LLMAPIError, HRAgentBaseError))