...
def _build_system_content(summary: str | None = None) -> str:
    ...
    summary 非空时追加历史交流摘要，帮助 LLM 保持多轮上下文连贯。
    """
...
async def classify_query_node(state: QAState) -> dict:
    ...
    try:
        ...
        thread_id = build_thread_id(state["candidate_id"], state["session_id"])
...
async def generate_rag_node(state: QAState) -> dict:
    ...
    for i, chunk in enumerate(ranked_chunks, 1):
        ...
        source_name = chunk.get("metadata", {}).get("source_name", "岗位文档")
...
async def generate_direct_node(state: QAState) -> dict:
    ...
    if web_sources:
        ...
        final_answer = (
            f"{answer_text}\n\n"
            f"⚠️ **说明**：以上为 AI 基于通用知识的回答，招聘知识库中暂无相关内容。"
            f"建议以 HR 讲解为准，或联系 HR 补充相关资料。"
        )
...
async def enqueue_pending_node(state: QAState) -> dict:
    """
    将低置信度问题写入 knowledge_pending_queue，供 HR 审查补充知识库。
                ...
                await session.execute(
                    text("""
                        INSERT INTO knowledge_pending_queue
                            (id, tenant_id, question, candidate_id, confidence, status)
                        VALUES (:id, :tenant_id, :question, :candidate_id, :confidence, 'pending')
                    ...
                    await session.execute(
                        text("""
                            INSERT INTO qa_sessions
                                (id, tenant_id, candidate_id, thread_id, summary, summary_version)
                            VALUES (:id, :tenant_id, :candidate_id, :thread_id, :summary, 1)
...
if __name__ == '__main__':
    ...
    # print(_rule_classify_specialized(query="AI岗位怎么样"))
    ...
    #          "tenant_id": "tenant_default",
    #          "candidate_id":"1efe1246-e355-4714-8b1b-bd4e7c8bce51", #必须在users库中出现
    #          "confidence": 0.74}
    #
    ...
    state = {"messages": [HumanMessage(content="模型训练的时候是怎么计算梯度的"),
                          AIMessage(content="反向传播的算法")],
             "original_query": "给我概括下",
             "candidate_id":"1efe1246-e355-4714-8b1b-bd4e7c8bce51",
             "tenant_id": "tenant_default",
             "session_id": "sess_001",
