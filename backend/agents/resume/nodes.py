...
async def save_results_node(state: ResumeState) -> dict:
    ...
    # 留一份完整结果给上层（API 可直接用）
    structured_output = {
        "review_id": review_id, "candidate_id": state["candidate_id"],
...
if __name__ == "__main__":
    ...
    async def main():
        async with AsyncSessionLocal() as s:        # 先插一条 processing 记录
            sid = (await s.execute(text("SELECT id FROM users WHERE username='candidate01'"))).scalar()
            await s.execute(text("""INSERT INTO resume_reviews (id,tenant_id,candidate_id,pdf_minio_path,status)
                VALUES (:id,'tenant_default',:sid,'resumes/x.pdf','processing')"""), {"id": rid, "sid": sid})
            await s.commit()
        with open("./resume.json") as f:
            state = json.load(f)

        dict1 = {"review_id": rid, "candidate_id": str(sid)}
        state.update(dict1)
